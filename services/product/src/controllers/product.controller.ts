...
export const getProduct = async (ctx: Koa.Context) => {
  ...
  if (!product) {
    ...
    ctx.state.natsPublishByFunc(
      publisherActivityCreate(ACTIVITY_TYPES.VIEW, product, ctx)
    );
    ctx.body = product;
    ctx.status = 200;
  }
...
export const getProducts = async (ctx: Koa.Context) => {
  ...
  if (query) {
    ctx.state.natsPublishByFunc(
      publisherActivityCreate(ACTIVITY_TYPES.SEARCH, ctx.request.query, ctx)
    );
  }
...
export const updateProduct = async (ctx: Koa.Context) => {
  ...
  await ProductModel.findOneAndUpdate({ _id: productId }, data).exec();
  ctx.status = 200;
};
